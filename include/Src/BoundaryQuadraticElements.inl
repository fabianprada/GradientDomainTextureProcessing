	...
	for (int c = 0; c < 2; c++) {
		for (int s = imin[c]; s <= imax[c]; s++) {
			...
			if (alpha > 0 && alpha < 1.0) {
				...
				int oppDimIndex = floor(position[1 - c] / oppCellSize);
				...
				if (boundaryNodeIndices.find(currentVertexKey) != boundaryNodeIndices.end() && boundaryNodeIndices.find(nextVertexKey) != boundaryNodeIndices.end()) {
					...
					if (boundarySegmentsInfo.find(segmentKey) != boundarySegmentsInfo.end()) {
						...
						if (oppHalfEdge != -1) {
							...
							for (int oppIter = oppositeIntersection.size() - 2; oppIter > 0; oppIter--) {
	...
	for (int i = 0; i < boundaryPolygons.size(); i++) {
		for (int j = 0; j < boundaryPolygons[i].size(); j++) {
			...
			if (delanauyTriangles.size() != currentPolygon.vertices.size() - 2) {
				...
				printf("Num Vertices %d \n", currentPolygon.vertices.size());
				printf("Num Face %d \n", delanauyTriangles.size());
			...
			for (int k = 0; k < delanauyTriangles.size(); k++) {
				...
				for (int v = 0; v < 3; v++) {
					...
					int previousPolygonVertex = (currentPolygonVertex + currentPolygon.vertices.size() - 1) % currentPolygon.vertices.size();
				...
				for (int v = 0; v < 3; v++) {
					...
					int edgeCorners[2] = { triangle.indices[(v + 1) % 3], triangle.indices[(v + 2) % 3] };
...
int InitializeCoarseBoundaryToFineBoundaryProlongation(const GridAtlas & gridAtlas, SparseMatrix<double, int> & coarseBoundaryFineBoundaryProlongation, std::vector<int> & fineBoundaryIndex, int & numFineBoundaryNodes, bool verbose = false) {
	...
	prolongationMatrix.resize(numFineBoundaryNodes, numBoundaryTexels);
	prolongationMatrix.setFromTriplets(prolongationTriplets.begin(), prolongationTriplets.end());

	SparseMatrixParser(prolongationMatrix, coarseBoundaryFineBoundaryProlongation, false);
