...
int DeepCoefficientRestriction(const std::vector<Real> & fineDeepCoefficients, std::vector<Real> & coarseDeepCoefficients, const std::vector<DeepLine> & deepLines) {
	...
	int numCoarseDeepCoeffs = coarseDeepCoefficients.size();
...
#pragma omp parallel for
	for (int t = 0; t < threads; t++) {
		...
		const int firstLine = (deepLines.size() * tId) / threads;
		const int lastLine = (deepLines.size() * (tId + 1)) / threads;
...
int BoundaryDeepMatrixConstruction(const int numBoundayTexels, const int numTexels, const std::vector<double> & deepCoefficients, const std::vector<BoundaryDeepIndex> & boundaryDeepIndices, SparseMatrix<double, int> & boundaryDeepMatrix) {
	...
	Eigen::SparseMatrix<double> _boundaryDeepMatrix;
	_boundaryDeepMatrix.resize(numBoundayTexels, numTexels);
	_boundaryDeepMatrix.setFromTriplets(boundaryDeepTriplets.begin(), boundaryDeepTriplets.end());

	SparseMatrixParser(_boundaryDeepMatrix, boundaryDeepMatrix, false);
...
int BoundaryBoundaryMatrixConstruction(const std::vector<int> & boundaryGlobalIndex, const int numBoundayTexels, const std::vector<double> & fineDeepCoefficients, const SparseMatrix<double, int> & fineBoundaryBoundaryMatrix, const SparseMatrix<double, int> & boundaryCoarseFineProlongation, const SparseMatrix<double, int> & boundaryFineCoarseRestriction, const std::vector<BoundaryBoundaryIndex> & boundaryBoundaryIndices, SparseMatrix<double, int> & coarseBoundaryBoundaryMatrix, bool verbose = false) {
	...
	Eigen::SparseMatrix<double> _boundaryBoundaryMatrix;
	_boundaryBoundaryMatrix.resize(numBoundayTexels, numBoundayTexels);
	_boundaryBoundaryMatrix.setFromTriplets(boundaryBoundaryTriplets.begin(), boundaryBoundaryTriplets.end());

	if (verbose) t_begin = clock();
	SparseMatrixParser(_boundaryBoundaryMatrix, coarseBoundaryBoundaryMatrix, false);
...
int FullMatrixConstruction(const GridAtlas & gridAtlas, const std::vector<double> & deepCoefficients, const SparseMatrix<double, int> & boundaryBoundaryMatrix, const SparseMatrix<double, int> & boundaryDeepMatrix, SparseMatrix<double, int> & fullMatrix) {
	...
	_boundaryMatrix.resize(numTexels, numTexels);
	_boundaryMatrix.setFromTriplets(boundaryTriplets.begin(), boundaryTriplets.end());

	SparseMatrix<double, int> boundaryMatrix;
	SparseMatrixParser(_boundaryMatrix, boundaryMatrix, false);
...
int UpdateMultigridCoefficients(const HierarchicalSystem & hierarchy, std::vector<MultigridLevelCoefficients<double>> & multigridCoefficients, const std::vector<double> & inputDeepCoefficients, const SparseMatrix<double, int> & inputBoundaryBoundaryMatrix, const SparseMatrix<double, int> & inputBoundaryDeepMatrix, const SparseMatrix<double, int> & systemMatrix, SparseMatrix<double, int> & coarseSystemMatrix, bool useDeepReciprocals, bool verbose = false) {
	...
	int levels = hierarchy.gridAtlases.size();
...
int UpdateLinearSystem( double stiffnessWeight, double screenWeight, const HierarchicalSystem & hierarchy, std::vector<MultigridLevelCoefficients<Real>> & multigridCoefficients,
	...
	int numDeepCoefficients = deepMassCoefficients.size();
...
#pragma omp parallel for
	...
	SparseMatrix<double, int> fineBoundaryBoundaryMatrix = boundaryBoundaryMassMatrix * screenWeight + boundaryBoundaryStiffnessMatrix * stiffnessWeight;
	SparseMatrix<double, int> fineBoundaryDeepMatrix = boundaryDeepMassMatrix * screenWeight + boundaryDeepStiffnessMatrix *  stiffnessWeight;
#if USE_FLOAT
	...
	for (int l = 0; l < levels; l++) {
		...
		for (int i = 0; i < _deepCoefficients.size(); i++)multigridCoefficients[l].deepCoefficients[i] = _deepCoefficients[i];
	}
#else
