#define USE_FLOAT 1
#include <Src/ScalarPrecisionType.inl>
...
{
	&Mesh,&Width,&Height,&StiffnessWeight,&CameraConfig, &Levels,&UseDirectSolver,&Threads,&DisplayMode,&MultigridBlockHeight,&MultigridBlockWidth,&MultigridPaddedHeight,&MultigridPaddedWidth,&MultigridUpdateVcycles,&Verbose,
...
template<class Real> double														Geodesics<Real>::stiffnessWeight;
...
//Samples
template<class Real> std::vector<HexagonalSampleInfo<Real>>							Geodesics<Real>::hexSamples;
template<class Real> std::vector<std::vector<CellLineSampleInfo<Real>>>				Geodesics<Real>::cellLineSamples;
...
#pragma omp parallel for
	...
	if (verbose) begin = clock();
	if (!IntergrateNormalizedVectorFieldFromSamples(Real(textureWidth),Real(textureHeight), interiorCellLines, cellLineSamples, hexSamples, exactSmoothImpulseSolution, fineGeodesicDistanceRHS, fineBoundaryValues, fineBoundaryRHS, true)) {
		printf("Unable to integrate normalized vector field! \n");
	}
	...
	if (state == GLUT_DOWN && glutGetModifiers() & GLUT_ACTIVE_SHIFT) {
		...
		if (impulseTexel != -1) {
			if (UseDirectSolver.set){
				clock_t begin;
				if (Verbose.set) begin = clock();
				ComputeExactSolution(Verbose.set);
				if (Verbose.set) printf("Exact solution %.4f \n", double(clock() - begin) / CLOCKS_PER_SEC);
...
#pragma omp parallel for
	...
	if (verbose) begin = clock();
	if (!IntergrateNormalizedVectorFieldFromSamples(Real(textureWidth), Real(textureHeight), interiorCellLines, cellLineSamples, hexSamples, multigridSmoothImpulseVariables[0].x, multigridGeodesicDistanceVariables[0].rhs, fineBoundaryValues, fineBoundaryRHS)) {
		printf("Unable to integrate normalized vector field! \n");
	}
	if (verbose) printf("Integrating normalized vector field %.4f \n", double(clock() - begin) / CLOCKS_PER_SEC);
...
int Geodesics<Real>::InitializeSystem(const int width, const int height) {
	...
	if (UseDirectSolver.set){
		...
		smoothImpulseMatrix = mass + stiffness * stiffnessWeight;
		geodesicDistanceMatrix = mass * 1e-6 + stiffness;
...
//////////////////////////////////// Initialize multigrid coefficients
	...
	if (!UpdateLinearSystem(stiffnessWeight,1.0, hierarchy, multigridSmoothImpulseCoefficients,
		...
		smoothImpulseMatrix, Verbose.set, true, UseDirectSolver.set)){
		printf("ERROR : Failed system update! \n");
		return 0;
	}

	if (!UpdateLinearSystem(1.0, 1e-6, hierarchy, multigridGeodesicDistanceCoefficients,
		deepMassCoefficients, deepStiffnessCoefficients,
		boundaryBoundaryMassMatrix, boundaryBoundaryStiffnessMatrix,
		boundaryDeepMassMatrix, boundaryDeepStiffnessMatrix,
		coarseGeodesicDistanceSolver, boundaryGeodesicDistanceSolver, fineGeodesicDistanceSolver,
		geodesicDistanceMatrix, Verbose.set, true, UseDirectSolver.set)) {
		printf("ERROR : Failed system update! \n");
		return 0;
	}
...
//////////////////////////////////// Initialize cell samples
	...
	cellLineSamples.resize(interiorCellLines.size());

	if (!InitializeNormalizedVectorFieldIntegration(parameterMetric,atlasCharts, hierarchy.gridAtlases[0].gridCharts, interiorCellLineIndex, fineBoundaryIndex, cellLineSamples, hexSamples)) {
		printf("Unable to initialize vector field integration samples! \n");
		return 0;
	}
	...
	for (int i = 0; i < cellLineSamples.size(); i++)  std::sort(cellLineSamples[i].begin(), cellLineSamples[i].end(), CellLineSampleCompare<Real>);
...
void Geodesics<Real>::InitializeVisualization(const int width, const int height) {
	...
	for (int i = 0; i <textureWidth*textureHeight; i++) visualization.textureImage[i] = Point3D<float>(0.5, 0.5, 0.5);
	for (int i = 0; i < textureNodes.size(); i++) {
		int ci = textureNodes[i].ci;
		int cj = textureNodes[i].cj;
		visualization.textureImage(ci, cj) = Point3D<float>(0.8, 0.8, 0.8);
	}
	visualization.UpdateTextureBuffer();
}

template<class Real>
int Geodesics<Real>::Init() {
	levels = Levels.value;
...
int _main(int argc, char* argv[])
{
	...
	if (DisplayMode.value == ONE_REGION_DISPLAY) {
		Geodesics<Real>::visualization.screenWidth = 800;
		Geodesics<Real>::visualization.screenHeight = 800;
	}
	else if (DisplayMode.value == TWO_REGION_DISPLAY) {
		Geodesics<Real>::visualization.screenWidth = 1440;
		Geodesics<Real>::visualization.screenHeight = 720;
	}
